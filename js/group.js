$(document).ready(function() {
    ...
    $('.begin-load-files').click(function() {
        ...
        var folder_id = urlVars['pf'];
          if (folder_id === undefined)
              folder_id = ajax_query({handler : 'get_group_folder_id', data : urlVars['id']}, 'group', 'json');
        ...
        $(file).each(function(index, item) {
            ...
            uploadTask[index] = pathReference.child('groups/' + path + item.name).put(item, {contentType: item.type});
    ...
    $('.remove_material').click(function() {
        var material_id = $(this).parents('.material').attr('data-material-id'),
            ...
            desertRef = pathReference.child("groups/" + path + mat_info.material_name + "." + mat_info.format);
    ...
    $('.create-folder').click(function() {
        ...
        creatingFolder = pathReference.child('groups/' + path + folder_name + '/init_folder_file.txt').putString("new folder");
    ...
    $('.remove_folder').click(function() {
        var folder_id = $(this).parents('.folder').attr('data-folder-id'),
            ...
            desertRef = pathReference.child("groups/" + path + "/init_folder_file.txt"),
    ...
    $('.request_action .accept').click(function() {
        var req_id = $(this).parents('tr').attr('data-request-id');
        var result = ajax_query({handler : 'accept_user_enter_in_group', data : req_id}, 'group', 'json');

        if (!result.result)
            alert_danger("Сталася помилка обробки заявки.", 3000);

        location.reload();
    });

    $('.request_action .cancel').click(function() {
        var req_id = $(this).parents('tr').attr('data-request-id');
        var result = ajax_query({handler : 'cancel_user_enter_in_group', data : req_id}, 'group', 'json');
    ...
    $('.user_action .cancel').click(function() {
        var student_id = $(this).parents('tr').attr('data-student-id');
        var result = ajax_query({handler : 'delete_user_from_group', data : student_id}, 'group', 'json');

        if (!result.result)
            alert_danger("Сталася помилка видалення користувача.", 3000);

        location.reload();
    });

    $('.create-news').click(function() {
        if (create_news('group', urlVars['id']))
